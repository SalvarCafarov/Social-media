import styles from './userBar.module.scss'
import { useRouter } from "next/router";

export default function UserBar({ setInputNick,user }) {
  const router = useRouter();

  function redirectProfile(userProp){
    router.push(`/user/${userProp.nickName}`)

  }
  return (
    <>
      <div className={styles.userBar } onClick={(e)=>{
        redirectProfile(user)
        setInputNick('')
      }} >
        <div className={styles.left}>
          <img src={user.image?user.image:"data:image/jpeg;base64,"} alt="" />
        </div>
        <div className={styles.right}>
          <div className={styles.nickName}>{user.nickName}</div>
          <div className={styles.bio}>{user.bio}</div>
        </div>
      </div>
    </>
  );
}
